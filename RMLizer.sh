...
if [[ $MODE_ARG == "htseq_only" ]]; then # test mode
    ...
    do
        ...
        htseq-count -o ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_hts_strand.bam -p BAM -r name -a 0 -f sam -s $STRANDEDNESS_ARG --nonunique $([[ $UNIQ_ARG -eq 1 ]] && echo "none" || echo "all") -i gene_id - $GTF_ARG > ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_counts.txt
...
#####################################
    ...
    do
        ...
        cat $FILE | sort -k 2n | uniq > ${OUTDIR_ARG}/${BAM_NAME}/${F_NAME}_sorted.txt && rm -f $FILE || echo "Error!"
        ...
        Rscript --vanilla -e 'args = commandArgs(trailingOnly = TRUE)
                    ...
                    file_process <- function(x){
                      ...
                      ret <- as_tibble(read.table(x,
                        ...
                        dplyr::group_by(chr, start, end, gene_id, strand, mismatch, is_dup) %>%
                        dplyr::summarize(mmm_count = n(), rname_lst = list(rname))
    ...
    cat $(ls -1 ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_breadth_per_gene_*.txt) > ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_breadth_per_gene.txt && rm -f $(ls -1 ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_breadth_per_gene_*.txt)
    ...
    pigz -h &> /dev/null && pigz -f -p $THR_ARG ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_breadth_per_gene.txt || gzip ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_breadth_per_gene.txt
    ...
    if [[ $RMLIZER_EXIT -eq 0 ]]; then
        ...
        if [ -f "${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_counts.txt.gz" ] ; then
            echo "Mismatch counts saved to ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_counts.txt.gz"
        fi

        if [ -f "${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer.bed.gz" ] ; then
            echo "Fragment coordinates BED file with T->C counts saved to ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer.bed.gz"
        fi

        if [ -f "${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_RMLizer_counts.txt" ] ; then
            echo "TC mismatch frequency data saved to ${OUTDIR_ARG}/${BAM_NAME}/${BAM_NAME}_mismatch_positions_data.rds"
        fi
